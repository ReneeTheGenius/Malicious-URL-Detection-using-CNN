...
tf.flags.DEFINE_string("checkpoint_dir", "./runs/1591084153/checkpoints/", "Checkpoint directory from training run")
...
if checkpoint_file is None:
    print("Cannot find a valid checkpoint file:{}! ".format(FLAGS.checkpoint_dir))
    exit(0)
...
if y_test is not None:
    ...
    for i in range(0,len(all_predictions)):
        if all_predictions[i]==bad and y_test[i][1]==bad:
            tp+=1
        elif all_predictions[i]==good and y_test[i][1]==good:
            tn+=1
        elif all_predictions[i]==bad and y_test[i][1]==good:
            fp+=1
        else :fn+=1
    ...
    print("Precision: {:g}".format(tp/float(tp+fp)))
    print("Recall: {:g}".format(tp/float(tp+fn)))
    precision = tp/float(tp+fp)
    recall = tp/float(tp+fn)
    print("F1: {:g}".format(2*precision*recall/(precision+recall)))
    print("FPR: {:g}".format(fp/float(tn+fp)))
    print("FNR: {:g}".format(fn/float(tp+fn)))
